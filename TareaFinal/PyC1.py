...
class Aplicacion:
    def __init__(self):
        ...
        self.alumnos = []
        self.crear_pestania_alta()
        self.crear_pestania_consultas()
        self.crear_menu()

        self.ventana1.mainloop()

    def crear_pestania_alta(self):
        pestania_alta = ttk.Frame(self.ventana1)
        pestania_alta.pack(fill="both", expand=1)

        ttk.Label(pestania_alta, text="Código:").grid(row=0, column=0, padx=5, pady=5)
        self.codigo_var = tk.StringVar()
        ttk.Entry(pestania_alta, textvariable=self.codigo_var).grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(pestania_alta, text="Nombre:").grid(row=1, column=0, padx=5, pady=5)
        self.nombre_var = tk.StringVar()
        ttk.Entry(pestania_alta, textvariable=self.nombre_var).grid(row=1, column=1, padx=5, pady=5)

        ttk.Label(pestania_alta, text="Apellidos:").grid(row=2, column=0, padx=5, pady=5)
        self.apellidos_var = tk.StringVar()
        ttk.Entry(pestania_alta, textvariable=self.apellidos_var).grid(row=2, column=1, padx=5, pady=5)

        ttk.Label(pestania_alta, text="Nota:").grid(row=3, column=0, padx=5, pady=5)
        self.nota_var = tk.StringVar()
        ttk.Entry(pestania_alta, textvariable=self.nota_var).grid(row=3, column=1, padx=5, pady=5)

        ttk.Button(pestania_alta, text="Guardar", command=self.guardar_alumno).grid(row=4, column=0, columnspan=2, padx=5, pady=5)

    def crear_pestania_consultas(self):
        pestania_consultas = ttk.Frame(self.ventana1)
        pestania_consultas.pack(fill="both", expand=1)

        ttk.Button(pestania_consultas, text="Consultar Aprobados", command=self.consultar_aprobados).pack(pady=5)
        ttk.Button(pestania_consultas, text="Consultar Suspensos", command=self.consultar_suspendidos).pack(pady=5)
